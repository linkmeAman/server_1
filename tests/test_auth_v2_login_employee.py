...
class TestAuthV2LoginEmployee(unittest.TestCase):
    ...
    def test_success_with_bcrypt_sidecar(self):
        ...
        with patch(
            "controllers.auth_v2.handlers.login_employee._resolve_main_identity",
            new=AsyncMock(return_value={"contact": {"id": 99}, "employee": {"id": 77}}),
        ), patch(
            "controllers.auth_v2.handlers.login_employee._resolve_central_identity",
            new=AsyncMock(return_value={"user": {"id": 12, "password": "x"}}),
        ), patch(
            "controllers.auth_v2.handlers.login_employee._load_lock_state",
            new=AsyncMock(return_value=None),
        ), patch(
            "controllers.auth_v2.handlers.login_employee._validate_password_and_maybe_migrate",
            new=AsyncMock(return_value=True),
        ), patch(
            "controllers.auth_v2.handlers.login_employee.AuthorizationResolver.resolve_employee_authorization",
            new=AsyncMock(
                return_value={
                    "position_id": 11,
                    "position": "Sales",
            ...
            "controllers.auth_v2.handlers.login_employee.issue_v2_token_pair",
            return_value={"access_token": "a", "refresh_token": "r", "jti": "j"},
        ), patch(
            "controllers.auth_v2.handlers.login_employee.write_audit_event",
            new=AsyncMock(),
        ):
    ...
    def test_plaintext_fallback_migration_log_emitted(self):
        async def _run():
            ...
            with patch(
                ...
                from controllers.auth_v2.handlers.login_employee import _validate_password_and_maybe_migrate
        ...
        for code, _label in scenarios:
            with self.subTest(code=code):
                with patch(
                    ...
                    "controllers.auth_v2.handlers.login_employee._resolve_central_identity",
                    new=AsyncMock(side_effect=AuthV2Error(code, "mismatch", 401)),
                ), patch(
                    "controllers.auth_v2.handlers.login_employee._record_failed_attempt",
    ...
    def test_employee_inactive_returns_auth_employee_inactive(self):
        ...
        with patch(
            "controllers.auth_v2.handlers.login_employee._resolve_main_identity",
            new=AsyncMock(side_effect=AuthV2Error(AUTH_EMPLOYEE_INACTIVE, "inactive", 403)),
        ), patch(
            "controllers.auth_v2.handlers.login_employee._record_failed_attempt",
            new=AsyncMock(return_value={"fail_count": 1, "locked_until": None}),
        ), patch(
