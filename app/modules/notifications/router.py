...
NOTIFICATIONS_ENABLED = get_settings().NOTIFICATIONS_ENABLED
...
if not NOTIFICATIONS_ENABLED:
    @router.get("/stream")
    async def stream_notifications_disabled(request: Request):
        response = Response(status_code=204)
        response.headers["X-Request-ID"] = _request_id(request)
        return response

    @router.get("/preferences")
    async def notification_preferences_disabled(request: Request):
        response = JSONResponse(
            status_code=200,
            content=success_response(
                data=NotificationPreferences().model_dump(mode="json"),
    ...
    async def patch_notification_preferences_disabled(
    ...
    @router.get("/rules")
    async def notification_rules_disabled(request: Request):
        response = JSONResponse(
            status_code=200,
            content=success_response(
                data={"results": [rule.model_dump(mode="json") for rule in []], "total": 0},
                message="Notifications disabled",
            ).model_dump(mode="json"),
        )
        response.headers["X-Request-ID"] = _request_id(request)
    ...
    @router.patch("/rules")
    async def patch_notification_rules_disabled(
        ...
        rule = NotificationDeliveryRule(
            ...
            reminder_offsets_minutes=payload.reminder_offsets_minutes or [5],
            recipient_scope=payload.recipient_scope or "assigned_to_me",
        )
        response = JSONResponse(
            status_code=200,
            content=success_response(
    ...
    @router.get("/recent")
    async def recent_notifications_disabled(
        request: Request,
    ...
    async def read_notification_disabled(event_id: str, request: Request):
        response = JSONResponse(
            status_code=200,
            content=success_response(
                data={"event_id": event_id, "read": False},
                message="Notifications disabled",
            ).model_dump(mode="json"),
        )
        response.headers["X-Request-ID"] = _request_id(request)
    ...
    async def read_all_notifications_disabled(request: Request):
        response = JSONResponse(
            status_code=200,
            content=success_response(
                data={"updated": 0},
    ...
    async def clear_all_visible_notifications_disabled(request: Request):
        response = JSONResponse(
            status_code=200,
            content=success_response(
                data={"updated": 0},
    ...
    async def clear_visible_notification_disabled(event_id: str, request: Request):
        response = JSONResponse(
            status_code=200,
            content=success_response(
                data={"event_id": event_id, "cleared": False},
                message="Notifications disabled",
            ).model_dump(mode="json"),
        )
        response.headers["X-Request-ID"] = _request_id(request)
    ...
    @router.post("/debug")
    async def publish_debug_notification_disabled(
        ...
        response.headers["X-Request-ID"] = payload.request_id or _request_id(request)
        return response
else:
    @router.get("/stream")
